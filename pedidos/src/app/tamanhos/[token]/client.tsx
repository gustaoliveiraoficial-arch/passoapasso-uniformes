...
import { buscarPedidoPorToken, salvarTamanhos, salvarTamanhosModelo } from '@/lib/firebase'
import type { Pedido, PieceEntry, CategoriaSize, ModeloPedido } from '@/types/pedido'
import { TAMANHOS_UNISSEX, TAMANHOS_BABYLOOK, TAMANHOS_INFANTIL, TAMANHOS_FEMININO, TAMANHOS_MASCULINO, calcularPreco } from '@/types/pedido'
import { formatarData, LOGO_URL } from '@/lib/utils'
...
// ── Types ──
type GeralLinha = { id: string; categoria: CategoriaSize; tamanho: string; quantidade: number; infoExtra?: string; ajustePreco?: number }
type PersonalizadoLinha = { id: string; pessoaNome: string; nomeNaCamiseta: string; numeroNaCamiseta: string; categoria: CategoriaSize; tamanho: string; infoExtra?: string; ajustePreco?: number }
type Aba = 'geral' | 'personalizado'

const CAT_LABEL: Record<CategoriaSize, string> = { unissex: 'Unissex', babylook: 'Baby Look', feminino: 'Feminino', masculino: 'Masculino', infantil: 'Infantil' }
const CAT_SHORT: Record<CategoriaSize, string> = { unissex: 'UNI', babylook: 'BL', feminino: 'FEM', masculino: 'MASC', infantil: 'INF' }

function getTamanhos(cat: CategoriaSize) {
  if (cat === 'infantil') return TAMANHOS_INFANTIL
  if (cat === 'babylook') return TAMANHOS_BABYLOOK
  if (cat === 'feminino') return TAMANHOS_FEMININO
  if (cat === 'masculino') return TAMANHOS_MASCULINO
...
export default function TamanhosCliente() {
  const { token: _paramToken } = useParams<{ token: string }>()
  const token = typeof window !== 'undefined'
    ? (window.location.pathname.split('/').filter(Boolean).pop() ?? _paramToken)
    : _paramToken
  ...
  // Multi-modelo: ID do modelo sendo editado
  const [modeloAtivo, setModeloAtivo] = useState<string | null>(null)
  // Tamanhos por modeloId (somente para pedidos multi-modelo)
  const [tamanhosModelos, setTamanhosModelos] = useState<Record<string, { geralLinhas: GeralLinha[], personalizados: PersonalizadoLinha[] }>>({})

  // Tamanhos para modo legado (1 modelo ou sem modelos[])
  ...
  useEffect(() => {
    buscarPedidoPorToken(token).then(p => {
      setPedido(p)
      setLoading(false)
      // Inicializa tamanhosModelos se multi-modelo
  ...
  // ── Tipo de referência do modelo ativo ──
  const isMultiModelo = !!(pedido?.modelos && pedido.modelos.length > 0)
  const dadosAtivos = isMultiModelo && modeloAtivo ? (tamanhosModelos[modeloAtivo] ?? { geralLinhas: [{ id: uuidv4(), categoria: 'unissex' as CategoriaSize, tamanho: '', quantidade: 1 }], personalizados: [] }) : null

  // ── Categorias disponíveis baseado em tipoReferencia do modelo ativo ──
  const modeloAtivoInfo = isMultiModelo
    ? pedido?.modelos?.find(m => m.id === modeloAtivo)
    : pedido?.modelos?.[0]
  const tipoRef = modeloAtivoInfo?.tipoReferencia
  const categoriasDisponiveis: CategoriaSize[] = tipoRef === 'babylook_unissex'
    ? ['unissex', 'babylook', 'infantil']
    : tipoRef === 'masculino_feminino'
      ? ['feminino', 'masculino', 'infantil']
      : ['unissex', 'babylook', 'feminino', 'masculino', 'infantil']
  const categoriaDefault: CategoriaSize = tipoRef === 'masculino_feminino' ? 'feminino' : 'unissex'

  function setDadosModelo(modeloId: string, updates: Partial<{ geralLinhas: GeralLinha[], personalizados: PersonalizadoLinha[] }>) {
    setTamanhosModelos(prev => ({
      ...prev,
      [modeloId]: { ...(prev[modeloId] ?? { geralLinhas: [], personalizados: [] }), ...updates },
    }))
  }

  // ── Geral ──
  const addGeral = () => {
    if (isMultiModelo && modeloAtivo) {
      const linhas = dadosAtivos?.geralLinhas ?? []
      setDadosModelo(modeloAtivo, { geralLinhas: [...linhas, { id: uuidv4(), categoria: categoriaDefault, tamanho: '', quantidade: 1 }] })
    } else {
      setGeralLinhas(p => [...p, { id: uuidv4(), categoria: categoriaDefault, tamanho: '', quantidade: 1 }])
    }
  }
  ...
  const updateGeral = (idx: number, campo: Partial<GeralLinha>) => {
    if (isMultiModelo && modeloAtivo) {
      const linhas = [...(dadosAtivos?.geralLinhas ?? [])]
      linhas[idx] = { ...linhas[idx], ...campo }
      if (campo.categoria) linhas[idx].tamanho = ''
      setDadosModelo(modeloAtivo, { geralLinhas: linhas })
    } else {
      setGeralLinhas(p => {
        const n = [...p]
        n[idx] = { ...n[idx], ...campo }
  ...
  const removeGeral = (idx: number) => {
    if (isMultiModelo && modeloAtivo) {
      const linhas = (dadosAtivos?.geralLinhas ?? []).filter((_, i) => i !== idx)
      setDadosModelo(modeloAtivo, { geralLinhas: linhas })
    } else {
      setGeralLinhas(p => p.filter((_, i) => i !== idx))
    }
  }
  ...
  const addPers = () => {
    if (isMultiModelo && modeloAtivo) {
      const pers = dadosAtivos?.personalizados ?? []
      setDadosModelo(modeloAtivo, { personalizados: [...pers, { id: uuidv4(), pessoaNome: '', nomeNaCamiseta: '', numeroNaCamiseta: '', categoria: categoriaDefault, tamanho: '' }] })
    } else {
      setPersonalizados(p => [...p, { id: uuidv4(), pessoaNome: '', nomeNaCamiseta: '', numeroNaCamiseta: '', categoria: categoriaDefault, tamanho: '' }])
    }
  }
  ...
  const updatePers = (idx: number, campo: Partial<PersonalizadoLinha>) => {
    if (isMultiModelo && modeloAtivo) {
      const pers = [...(dadosAtivos?.personalizados ?? [])]
      pers[idx] = { ...pers[idx], ...campo }
      if (campo.categoria) pers[idx].tamanho = ''
      setDadosModelo(modeloAtivo, { personalizados: pers })
    } else {
      setPersonalizados(p => {
        const n = [...p]
        n[idx] = { ...n[idx], ...campo }
  ...
  const removePers = (idx: number) => {
    if (isMultiModelo && modeloAtivo) {
      const pers = (dadosAtivos?.personalizados ?? []).filter((_, i) => i !== idx)
      setDadosModelo(modeloAtivo, { personalizados: pers })
    } else {
      setPersonalizados(p => p.filter((_, i) => i !== idx))
    }
  }

  // ── Arrays ativos (depende do modo) ──
  const geralLinhasAtivas = isMultiModelo && modeloAtivo ? (dadosAtivos?.geralLinhas ?? []) : geralLinhas
  const personalizadosAtivos = isMultiModelo && modeloAtivo ? (dadosAtivos?.personalizados ?? []) : personalizados

  // ── Totais ──
  // Para multi-modelo, contar todas as peças configuradas em todos os modelos
  const totalPecas = pedido?.quantidadeTotal || 0
  const totalConfMulti = isMultiModelo
    ? Object.values(tamanhosModelos).reduce((sum, dados) => {
        const g = dados.geralLinhas.reduce((s, l) => s + (l.tamanho ? Number(l.quantidade) || 0 : 0), 0)
        const p = dados.personalizados.filter(p => p.tamanho).length
        return sum + g + p
      }, 0)
    : 0
  const totalGeral = geralLinhasAtivas.reduce((s, l) => s + (l.tamanho ? Number(l.quantidade) || 0 : 0), 0)
  const totalPers = personalizadosAtivos.filter(p => p.tamanho).length
  const totalConf = isMultiModelo ? totalConfMulti : (totalGeral + totalPers)
  const ok = totalConf === totalPecas && totalPecas > 0
  const pct = totalPecas > 0 ? Math.min(100, (totalConf / totalPecas) * 100) : 0

  function buildPecasFromDados(dados: { geralLinhas: GeralLinha[], personalizados: PersonalizadoLinha[] }, pedidoData: Pedido, modelo: ModeloPedido): PieceEntry[] {
    const precoBase = modelo.precoUnitario || pedidoData.precoUnitario
    const r: PieceEntry[] = []
    for (const l of dados.geralLinhas.filter(l => l.tamanho && Number(l.quantidade) > 0))
      for (let i = 0; i < Number(l.quantidade); i++)
        r.push({ id: uuidv4(), pessoaNome: '', nomeNaCamiseta: '', numeroNaCamiseta: '', categoria: l.categoria, tamanho: l.tamanho, infoExtra: l.infoExtra, precoUnitario: calcularPreco(precoBase, l.categoria, l.tamanho) + (l.ajustePreco || 0) })
    for (const p of dados.personalizados.filter(p => p.tamanho))
      r.push({ id: uuidv4(), pessoaNome: p.pessoaNome, nomeNaCamiseta: p.nomeNaCamiseta, numeroNaCamiseta: p.numeroNaCamiseta, categoria: p.categoria, tamanho: p.tamanho, infoExtra: p.infoExtra, precoUnitario: calcularPreco(precoBase, p.categoria, p.tamanho) + (p.ajustePreco || 0) })
    return r
  }
  ...
  function buildPecas(): PieceEntry[] {
    ...
    for (const l of geralLinhas.filter(l => l.tamanho && Number(l.quantidade) > 0))
      for (let i = 0; i < Number(l.quantidade); i++)
        r.push({ id: uuidv4(), pessoaNome: '', nomeNaCamiseta: '', numeroNaCamiseta: '', categoria: l.categoria, tamanho: l.tamanho, infoExtra: l.infoExtra, precoUnitario: calcularPreco(pedido.precoUnitario, l.categoria, l.tamanho) + (l.ajustePreco || 0) })
    for (const p of personalizados.filter(p => p.tamanho))
      r.push({ id: uuidv4(), pessoaNome: p.pessoaNome, nomeNaCamiseta: p.nomeNaCamiseta, numeroNaCamiseta: p.numeroNaCamiseta, categoria: p.categoria, tamanho: p.tamanho, infoExtra: p.infoExtra, precoUnitario: calcularPreco(pedido.precoUnitario, p.categoria, p.tamanho) + (p.ajustePreco || 0) })
    return r
  }
  ...
  async function confirmar() {
    ...
    setSalvando(true)
    try {
      if (isMultiModelo && pedido.modelos && pedido.modelos.length > 0) {
        for (const modelo of pedido.modelos) {
          const dados = tamanhosModelos[modelo.id]
          if (dados) {
            const pecas = buildPecasFromDados(dados, pedido, modelo)
            await salvarTamanhosModelo(pedido.id, modelo.id, pecas)
          }
        }
      } else {
  ...
  // ── Resumo / Confirmação ──
  if (mostrarResumo) {
    // Para multi-modelo, construir pecas para resumo a partir de todos os modelos
    const pecas = isMultiModelo && pedido?.modelos && pedido.modelos.length > 0
      ? pedido.modelos.flatMap(m => {
          const dados = tamanhosModelos[m.id]
          return dados ? buildPecasFromDados(dados, pedido, m) : []
        })
      : buildPecas()
    const grupos: Record<string, number> = {}
    pecas.forEach(p => { const k = `${CAT_LABEL[p.categoria]} ${p.tamanho}`; grupos[k] = (grupos[k] || 0) + 1 })
    // Para resumo de personalizados no modo multi-modelo, concatenar todos
    const personalizadosResumo = isMultiModelo && pedido?.modelos
      ? pedido.modelos.flatMap(m => tamanhosModelos[m.id]?.personalizados ?? [])
      : personalizados
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-4 pb-32">
          ...
          {personalizadosResumo.filter(p => p.tamanho).length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {personalizadosResumo.filter(p => p.tamanho).map((p, i) => (
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col" style={{ WebkitTapHighlightColor: 'transparent' }}>
      ...
      <div className="bg-orange-500 text-white text-center px-4 pt-5 pb-4">
        <img src={LOGO_URL} alt="Passo a Passo Uniformes" className="h-10 w-auto mx-auto mb-1.5 brightness-0 invert" />
      ...
      {/* Seletor de modelos (multi-modelo) */}
      {isMultiModelo && pedido.modelos && pedido.modelos.length > 1 && (
        <div className="px-4 py-3 bg-white border-b border-gray-100">
          <p className="text-xs font-bold text-gray-400 uppercase mb-3">Selecione o produto para preencher os tamanhos:</p>
          <div className="flex flex-wrap gap-2">
            {pedido.modelos.map((m, i) => {
              const dadosM = tamanhosModelos[m.id]
              const temPecas = dadosM?.geralLinhas?.some(l => l.quantidade > 0 && l.tamanho) || (dadosM?.personalizados?.some(p => p.tamanho) ?? false)
              return (
                <button key={m.id}
                ...
                >
                  {i + 1}. {CATALOGO[m.modelo]?.label || m.modelo}
                  {temPecas && <span className="ml-1 text-xs">✓</span>}
                </button>
              )
            })}
          </div>
      ...
      <div className="flex-1 overflow-y-auto pb-32">
        ...
        {aba === 'geral' && (
          <div className="px-4 pt-4 space-y-3">
            ...
            {geralLinhasAtivas.map((linha, idx) => {
                ...
                <div key={linha.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <div className="px-4 py-4 space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        className="w-full text-center border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:border-blue-400 focus:outline-none bg-white"
                        placeholder="Ex: cor diferente, detalhe especial..."
                        value={linha.infoExtra || ''}
                        onChange={e => updateGeral(idx, { infoExtra: e.target.value })}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <button type="button"
                          onClick={() => { const v = Math.abs(linha.ajustePreco || 0); updateGeral(idx, { ajustePreco: v > 0 ? -v : 0 }) }}
                          className={`px-3 py-2 rounded-xl text-xs font-bold border-2 transition ${(linha.ajustePreco || 0) < 0 ? 'bg-red-500 text-white border-red-500' : 'border-gray-200 text-gray-600 bg-white'}`}>
                          − Desconto
                        </button>
                        <input
                          type="number" min={0} step={1}
                          className="w-20 text-center border-2 border-gray-200 rounded-xl px-2 py-2 text-base font-black focus:border-orange-400 focus:outline-none"
                          placeholder="0"
                          value={linha.ajustePreco ? Math.abs(linha.ajustePreco) : ''}
                          onChange={e => {
                            const val = parseFloat(e.target.value) || 0
                            const sign = (linha.ajustePreco || 0) <= 0 ? -1 : 1
                            updateGeral(idx, { ajustePreco: val === 0 ? 0 : val * sign })
                          }}
                        />
                        <button type="button"
                          onClick={() => { const v = Math.abs(linha.ajustePreco || 0); updateGeral(idx, { ajustePreco: v > 0 ? v : 0 }) }}
                          className={`px-3 py-2 rounded-xl text-xs font-bold border-2 transition ${(linha.ajustePreco || 0) > 0 ? 'bg-green-500 text-white border-green-500' : 'border-gray-200 text-gray-600 bg-white'}`}>
                          + Acréscimo
                        </button>
                      </div>
                      {(linha.ajustePreco || 0) !== 0 && (
                        <p className={`text-center text-xs mt-1.5 font-bold ${(linha.ajustePreco || 0) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {(linha.ajustePreco || 0) > 0 ? `+R$ ${(linha.ajustePreco || 0).toFixed(2)} por peça` : `-R$ ${Math.abs(linha.ajustePreco || 0).toFixed(2)} por peça`}
                        </p>
                      )}
                    </div>
                  </div>
        ...
        {aba === 'personalizado' && (
          <div className="px-4 pt-4 space-y-3">
            ...
            {personalizadosAtivos.map((ind, idx) => {
                ...
                <div key={ind.id} className="bg-white rounded-2xl border border-purple-100 shadow-sm overflow-hidden">
                  ...
                  <div className="px-4 py-4 space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        className="w-full text-center border-2 border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:border-purple-400 focus:outline-none bg-white"
                        placeholder="Ex: cor diferente, detalhe especial..."
                        value={ind.infoExtra || ''}
                        onChange={e => updatePers(idx, { infoExtra: e.target.value })}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <button type="button"
                          onClick={() => { const v = Math.abs(ind.ajustePreco || 0); updatePers(idx, { ajustePreco: v > 0 ? -v : 0 }) }}
                          className={`px-3 py-2 rounded-xl text-xs font-bold border-2 transition ${(ind.ajustePreco || 0) < 0 ? 'bg-red-500 text-white border-red-500' : 'border-gray-200 text-gray-600 bg-white'}`}>
                          − Desconto
                        </button>
                        <input
                          type="number" min={0} step={1}
                          className="w-20 text-center border-2 border-gray-200 rounded-xl px-2 py-2 text-base font-black focus:border-orange-400 focus:outline-none"
                          placeholder="0"
                          value={ind.ajustePreco ? Math.abs(ind.ajustePreco) : ''}
                          onChange={e => {
                            const val = parseFloat(e.target.value) || 0
                            const sign = (ind.ajustePreco || 0) <= 0 ? -1 : 1
                            updatePers(idx, { ajustePreco: val === 0 ? 0 : val * sign })
                          }}
                        />
                        <button type="button"
                          onClick={() => { const v = Math.abs(ind.ajustePreco || 0); updatePers(idx, { ajustePreco: v > 0 ? v : 0 }) }}
                          className={`px-3 py-2 rounded-xl text-xs font-bold border-2 transition ${(ind.ajustePreco || 0) > 0 ? 'bg-green-500 text-white border-green-500' : 'border-gray-200 text-gray-600 bg-white'}`}>
                          + Acréscimo
                        </button>
                      </div>
                      {(ind.ajustePreco || 0) !== 0 && (
                        <p className={`text-center text-xs mt-1.5 font-bold ${(ind.ajustePreco || 0) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {(ind.ajustePreco || 0) > 0 ? `+R$ ${(ind.ajustePreco || 0).toFixed(2)} por peça` : `-R$ ${Math.abs(ind.ajustePreco || 0).toFixed(2)} por peça`}
                        </p>
                      )}
                    </div>
                  </div>
