  ...
  return (
    <>
      ...
      <div className="cliente-folha bg-white mx-auto my-6 shadow-lg print:shadow-none print:my-0">
        ...
        <div className="mb-6">
          ...
          {isEmpresa ? (
            <div className="grid grid-cols-2 gap-x-8 gap-y-2 border border-gray-200 rounded-xl p-4 bg-gray-50/50">
              ...
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold">Inscrição Estadual</p>
                <p className="text-sm font-semibold text-gray-800">{empresa?.inscricaoEstadual || '—'}</p>
              </div>
              <div>
        ...
        {/* ── PAGAMENTO ── */}
        {(pedido.descricaoPagamento || (pedido.recibosPagamento?.length ?? 0) > 0) && (
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-5 h-5 rounded bg-orange-500 flex items-center justify-center">
                <span className="text-white text-xs font-bold">6</span>
              </div>
              <h2 className="font-bold text-gray-900 text-sm uppercase tracking-wide">Pagamento</h2>
            </div>
            <div className="border border-gray-200 rounded-xl p-4 bg-gray-50/50 space-y-3">
              {pedido.descricaoPagamento && (
                <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold mb-1">Observações do Pagamento</p>
                  <p className="text-sm text-gray-800 whitespace-pre-wrap leading-relaxed">{pedido.descricaoPagamento}</p>
                </div>
              )}
              {(pedido.recibosPagamento?.length ?? 0) > 0 && (
                <div>
                  <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold mb-2">Comprovante(s) de Pagamento</p>
                  <div className="flex flex-wrap gap-3">
                    {pedido.recibosPagamento!.map((url, i) => (
                      <div key={i} className="text-center">
                        <img src={url} alt={`Comprovante ${i + 1}`}
                          className="w-28 h-28 object-cover rounded-lg border border-gray-200" />
                        <p className="text-xs text-gray-400 mt-1">Rec. {i + 1}</p>
                      </div>
                    ))}
                  </div>
                </div>
