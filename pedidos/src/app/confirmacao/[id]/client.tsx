        ...
        <div style={{ flex: '0 0 50%', display: 'flex', flexDirection: 'column', gap: '3mm', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <img src={LOGO_URL} alt="Passo a Passo Uniformes" style={{ height: 40, width: 'auto' }} />
            </div>
          ...
          <div style={{ marginTop: 'auto' }}>
            ...
            {((pedido.recibosPagamento?.length ?? 0) > 0 || pedido.descricaoPagamento) && (
              <div style={{ marginTop: 5, paddingTop: 4, borderTop: '1.5px solid #e5e7eb' }}>
                <div style={{ fontSize: 7.5, fontWeight: 700, color: '#E8500A', letterSpacing: 1, marginBottom: 3 }}>PAGAMENTO:</div>
                {pedido.descricaoPagamento && (
                  <div style={{ fontSize: 7, color: '#333', lineHeight: 1.5, marginBottom: 4, maxHeight: 28, overflow: 'hidden' }}>
                    {pedido.descricaoPagamento}
                  </div>
                )}
                {(pedido.recibosPagamento?.length ?? 0) > 0 && (
                  <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                    {pedido.recibosPagamento!.slice(0, 4).map((url, i) => {
                      const isPdf = url.includes('/raw/') || url.toLowerCase().endsWith('.pdf')
                      return isPdf ? (
                        <div key={i} style={{ width: 32, height: 32, background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>📄</div>
                      ) : (
                        <img key={i} src={url} alt={`Recibo ${i + 1}`} crossOrigin="anonymous"
                          style={{ width: 32, height: 32, objectFit: 'cover', borderRadius: 3, border: '1px solid #e5e7eb' }} />
                      )
                    })}
                    {(pedido.recibosPagamento!.length > 4) && (
                      <div style={{ width: 32, height: 32, background: '#f3f4f6', border: '1px solid #e5e7eb', borderRadius: 3, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7, fontWeight: 700, color: '#6b7280' }}>
                        +{pedido.recibosPagamento!.length - 4}
                      </div>
                    )}
                  </div>
