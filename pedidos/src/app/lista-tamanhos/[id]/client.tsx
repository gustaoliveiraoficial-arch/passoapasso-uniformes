...
export default function ListaTamanhosPage() {
  ...
  // Coleta todas as peças (suporte a multi-modelo + legado)
  const todasPecas: typeof pedido.pecas = pedido.modelos && pedido.modelos.length > 0
    ? pedido.modelos.flatMap(m => m.pecas || [])
    : pedido.pecas || []
  const pecas = todasPecas
  const total = pecas.reduce((s, p) => s + p.precoUnitario, 0)

  // Peças com personalização (nome/número) — usadas na folha extra
  const pecasPersonalizadas = pecas.filter(p => p.pessoaNome || p.nomeNaCamiseta || p.numeroNaCamiseta)
  const temPersonalizado = pecasPersonalizadas.length > 0
  ...
  return (
    <>
      ...
      <div className="lista-folha bg-white mx-auto my-6 shadow-lg print:shadow-none print:my-0">
        ...
        <div className="lista-header flex items-center justify-between pb-4 border-b-2 border-orange-500 mb-5">
          <div className="flex items-center gap-3">
            <img src={LOGO_URL} alt="Passo a Passo" style={{ height: 44 }} />
            <div>
      ...
      {/* ── FOLHA EXTRA PAISAGEM — Peças Personalizadas ── */}
      {temPersonalizado && (
        <div className="lista-folha-paisagem bg-white print:shadow-none">

          {/* Cabeçalho */}
          <div className="flex items-center justify-between pb-3 border-b-2 border-orange-500 mb-4">
            <div className="flex items-center gap-3">
              <img src={LOGO_URL} alt="Passo a Passo" style={{ height: 36 }} />
              <div>
                <div className="text-xs text-gray-500 font-semibold uppercase tracking-wide">Passo a Passo Uniformes</div>
                <div className="text-xs text-gray-400 font-bold text-orange-600">Folha de Personalizações</div>
              </div>
            </div>
            <div className="text-right">
              <div className="text-base font-bold text-gray-900">Pedido #{pedido.numeroPedido}</div>
              <div className="text-xs text-gray-500">{nomeCliente}</div>
          ...
          <table className="lista-tabela w-full mb-4" style={{ fontSize: 11 }}>
            <thead>
              <tr className="bg-orange-500 text-white">
                <th className="lista-th text-center w-8">#</th>
                <th className="lista-th">Quem vai usar</th>
                <th className="lista-th">Nome na peça</th>
                <th className="lista-th text-center">Nº na peça</th>
                <th className="lista-th text-center">Tamanho</th>
                <th className="lista-th">Observação da peça</th>
                <th className="lista-th text-right">Ajuste (R$)</th>
              </tr>
            </thead>
            <tbody>
              {pecasPersonalizadas.map((peca, idx) => {
                const ajuste = peca.ajustePreco ?? 0
                return (
                  <tr key={peca.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="lista-td text-center text-gray-500">{idx + 1}</td>
                    <td className="lista-td font-semibold">{peca.pessoaNome || '—'}</td>
                    <td className="lista-td font-bold text-orange-700">{peca.nomeNaCamiseta || '—'}</td>
                    <td className="lista-td text-center font-bold">{peca.numeroNaCamiseta || '—'}</td>
                    <td className="lista-td text-center font-bold">{CATEGORIA_LABEL[peca.categoria] || peca.categoria} {peca.tamanho}</td>
                    <td className="lista-td text-gray-600 text-xs">{peca.infoExtra || '—'}</td>
                    <td className={`lista-td text-right font-bold ${ajuste > 0 ? 'text-green-700' : ajuste < 0 ? 'text-red-600' : 'text-gray-400'}`}>
                      {ajuste === 0 ? '—' : ajuste > 0 ? `+R$ ${ajuste.toFixed(2)}` : `-R$ ${Math.abs(ajuste).toFixed(2)}`}
                    </td>
                  </tr>
                )
              })}
            ...
            <tfoot>
              <tr className="bg-gray-100">
                <td colSpan={6} className="lista-td text-right font-bold text-gray-600">
                  Total de peças personalizadas: {pecasPersonalizadas.length}
                </td>
                <td className="lista-td text-right font-bold text-gray-800">
                  {(() => {
                    const totalAjuste = pecasPersonalizadas.reduce((s, p) => s + (p.ajustePreco ?? 0), 0)
                    return totalAjuste === 0 ? '—' : totalAjuste > 0 ? `+R$ ${totalAjuste.toFixed(2)}` : `-R$ ${Math.abs(totalAjuste).toFixed(2)}`
                  })()}
                </td>
              </tr>
            </tfoot>
          ...
          {pedido.observacao && (
            <div className="mb-4 px-4 py-3 bg-amber-50 border border-amber-200 rounded">
              <p className="text-xs font-bold text-amber-700 uppercase tracking-wide mb-1">Observações do Pedido</p>
              <p className="text-sm text-amber-900 leading-relaxed">{pedido.observacao}</p>
            </div>
          )}

          {/* Descontos / Acréscimos gerais de negociação */}
          {(pedido.valorNegociacao != null && pedido.valorNegociacao !== 0) && (
          ...
          {/* Rodapé */}
          <div className="border-t border-gray-200 pt-3 flex items-end justify-between mt-auto">
            <div className="text-xs text-gray-400">
              <p>Passo a Passo Uniformes — Folha de Personalizações</p>
              <p>Gerado em: {new Date().toLocaleString('pt-BR')}</p>
            </div>
            <div className="text-xs text-gray-400 text-right">
              <p>Pedido #{pedido.numeroPedido} · Vendedor: {pedido.nomeVendedor}</p>
            </div>
          </div>
        </div>
      )}
