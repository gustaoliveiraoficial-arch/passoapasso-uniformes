...
import { buscarPedido, uploadArquivo, atualizarPedido, atualizarStatus, atualizarModeloArquivos } from '@/lib/firebase'
...
import { formatarData, urlVerPDF, LOGO_URL } from '@/lib/utils'
...
export default function ArquivosArteFinalista() {
  ...
  // Seletor de modelo (multi-modelo)
  const [modeloAtivo, setModeloAtivo] = useState<string>('')
  ...
  async function carregar() {
    ...
    } else if (p?.estampaQuadros?.length) {
      setQuadros(p.estampaQuadros)
    } else {
      setQuadros([])
    }
    setLoading(false)
  ...
  async function handleUpload(files: FileList | File[], pasta: 'layouts' | 'vetores') {
    ...
    if (novas.length > 0) {
      ...
      if (pedido.modelos && pedido.modelos.length > 0 && modeloAtivo) {
        ...
        await atualizarModeloArquivos(id, modeloAtivo, { [campo]: [...atualModelo, ...novas] })
      } else {
  ...
  function handlePasteQuadro(e: React.ClipboardEvent, qid: string) {
  ...
  async function salvarQuadros() {
    ...
    try {
      if (pedido.modelos && pedido.modelos.length > 0 && modeloAtivo) {
        await atualizarModeloArquivos(id, modeloAtivo, { estampaQuadros: quadros })
      } else {
  ...
  // Para multi-modelo, usar dados do modelo ativo; senão, dados top-level
  const modeloAtualObj = isMultiModelo && modeloAtivo ? pedido.modelos!.find(m => m.id === modeloAtivo) ?? pedido.modelos![0] : null
  const modeloLabel = modeloAtualObj ? (CATALOGO[modeloAtualObj.modelo]?.label || modeloAtualObj.modelo) : (CATALOGO[pedido.modelo]?.label || pedido.modelo)
  const materialLabel = modeloAtualObj ? (CATALOGO[modeloAtualObj.modelo]?.materiais[modeloAtualObj.material]?.label || modeloAtualObj.material) : (CATALOGO[pedido.modelo]?.materiais[pedido.material]?.label || pedido.material)
  const corFinal = modeloAtualObj ? (modeloAtualObj.cor === 'Personalizada' ? modeloAtualObj.corPersonalizada : modeloAtualObj.cor) : (pedido.cor === 'Personalizada' ? pedido.corPersonalizada : pedido.cor)
  const estampas = (modeloAtualObj?.tiposEstampa ?? pedido.tiposEstampa)?.map(t => TIPOS_ESTAMPA.find(e => e.value === t)?.label).filter(Boolean).join(', ')
  const detalhesAtivos = (['corpo', 'friso', 'punho', 'gola', 'manga', 'bolso'] as const).filter(d => (modeloAtualObj?.detalhes ?? pedido.detalhes)?.[d])

  // Layouts e vetores do modelo ativo (ou top-level)
  const layoutImagesAtivos = modeloAtualObj ? (modeloAtualObj.layoutImages ?? []) : (pedido.layoutImages ?? [])
  const vetoresFilesAtivos = modeloAtualObj ? (modeloAtualObj.vetoresFiles ?? []) : (pedido.vetoresFiles ?? [])
  const temLayouts = layoutImagesAtivos.length > 0
  const temVetores = vetoresFilesAtivos.length > 0

  // Peças do modelo ativo (ou top-level)
  const pecasAtivas = modeloAtualObj ? (modeloAtualObj.pecas ?? []) : (pedido.pecas ?? [])

  // Totais financeiros
  const totalPedido = pedido.pecas?.reduce((s, p) => s + p.precoUnitario, 0) ?? (pedido.precoUnitario * pedido.quantidadeTotal)

  // Agrupa tamanhos (do modelo ativo)
  ...
  const comPersonalizacao = pecasAtivas.filter(p => p.nomeNaCamiseta || p.numeroNaCamiseta)

  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center gap-3">
          <img src={LOGO_URL} alt="Passo a Passo Uniformes" className="h-9 w-auto flex-shrink-0" />
          <div>
      ...
      <main className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        ...
        {isMultiModelo && pedido.modelos && pedido.modelos.length > 1 && (
          <div className="bg-white rounded-2xl border border-gray-200 p-4">
            <p className="text-xs font-bold text-gray-400 uppercase mb-3">Modelo ativo:</p>
            <div className="flex flex-wrap gap-2">
              {pedido.modelos.map((m, i) => (
                <button key={m.id} onClick={() => {
                  setModeloAtivo(m.id)
                  // Atualiza quadros para o novo modelo selecionado
                  if (m.estampaQuadros?.length) setQuadros(m.estampaQuadros)
                  else setQuadros([])
                }}
                  className={`px-4 py-2 rounded-xl text-sm font-bold border-2 transition ${
                    modeloAtivo === m.id ? 'bg-orange-500 text-white border-orange-500' : 'bg-white text-gray-700 border-gray-200'
                  }`}
                >
            ...
            {quadros.map((q, qi) => (
              <div key={q.id} className="border-2 border-gray-100 rounded-2xl overflow-hidden">
                ...
                <div className="bg-gray-50 px-4 py-3 flex items-center gap-3 border-b border-gray-100">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-gray-400 font-semibold uppercase tracking-wide mb-1">Posição da estampa</p>
                    <select value={q.posicao} onChange={e => updateQuadro(q.id, { posicao: e.target.value })}
                      className="w-full text-sm font-bold bg-white border-2 border-blue-300 rounded-lg px-3 py-1.5 focus:outline-none focus:border-blue-500 text-gray-800 cursor-pointer">
                      {POSICOES_ESTAMPA.map(p => <option key={p} value={p}>{p}</option>)}
                    </select>
                  </div>
                  <button onClick={() => removeQuadro(q.id)} className="p-1 text-red-400 hover:text-red-600 mt-4"><X size={16} /></button>
                </div>
                ...
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">Imagem da Estampa</p>
                    <p className="text-[10px] text-blue-500 mb-2 font-semibold">📋 Ctrl+V para colar print</p>
                    {q.imagem ? (
                      (() => {
                        const ext = q.imagem.split('?')[0].split('.').pop()?.toLowerCase() || ''
                        const isImg = ['jpg','jpeg','png','webp','gif','svg'].includes(ext)
                        return (
                          <div
                            className="relative group outline-none"
                            tabIndex={0}
                            onPaste={e => handlePasteQuadro(e, q.id)}
                            title="Ctrl+V para colar nova imagem"
                          >
                            {isImg ? (
                              ...
                              <div className="w-full aspect-square rounded-xl border-2 border-blue-100 bg-blue-50 flex flex-col items-center justify-center gap-2">
                                <FileText size={36} className="text-blue-400" />
                                <span className="text-xs font-bold text-blue-600 uppercase">{ext || 'arquivo'}</span>
                                <span className="text-xs text-gray-500 text-center px-2 truncate max-w-full">
                                  {decodeURIComponent(q.imagem.split('/').pop()?.split('?')[0] || '')}
                                </span>
                              </div>
                            )}
                            <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition">
                              <a href={q.imagem} target="_blank" rel="noopener noreferrer" download
                      ...
                      <>
                        <label
                          htmlFor={`quadro-img-${q.id}`}
                          tabIndex={0}
                          onPaste={e => handlePasteQuadro(e, q.id)}
                          className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl aspect-square cursor-pointer transition outline-none ${uploadandoQuadroId === q.id ? 'border-blue-300 bg-blue-50/50 cursor-wait' : 'border-blue-200 hover:border-blue-400 hover:bg-blue-50/30 focus:border-blue-400 focus:bg-blue-50/30'}`}>
                          {uploadandoQuadroId === q.id ? (
                            ...
                            <>
                              ...
                              <span className="text-xs text-gray-500 text-center px-2">Clique para enviar ou Ctrl+V para colar</span>
                            </>
                          )}
                        </label>
                        <input
                          ...
                          type="file"
                          accept="image/png,image/jpeg,image/jpg,image/webp,image/*,.pdf,.cdr,.ai,.eps,.svg,application/pdf,application/postscript"
                          className="sr-only"
                          onChange={e => {
                            if (e.target.files?.[0]) {
        ...
        <div className="bg-white rounded-2xl border-2 border-orange-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-orange-100 flex items-center gap-3">
            ...
            {temLayouts && <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-xs font-bold rounded-full">{layoutImagesAtivos.length}</span>}
          </div>
        ...
        <div className="bg-white rounded-2xl border-2 border-purple-200 shadow-sm overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            <div className="bg-purple-50 rounded-xl p-3 flex items-start gap-2">
              ...
              <p className="text-xs text-purple-700">Formatos aceitos: <strong>PDF</strong>, <strong>CorelDraw (.cdr)</strong> e <strong>Illustrator (.ai/.eps)</strong></p>
            </div>
            ...
            <input id="upload-vetores" type="file" accept=".pdf,.cdr,.ai,.eps,.svg,application/pdf,application/postscript" multiple className="sr-only"
                ...
                {vetoresFilesAtivos.map((url, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200">
                      ...
                      <a href={urlVerPDF(url)} download rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 flex-shrink-0"><ExternalLink size={16} /></a>
